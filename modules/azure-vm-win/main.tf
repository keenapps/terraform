...
resource "azurerm_windows_virtual_machine" "this" {
  for_each = var.instances != {} ? var.instances : { "default" = local.single_config }

  name                = "${var.name}-${each.key}-${module.suffix.result}"
  computer_name       = try(var.computer_name_prefix, "") != "" ? "${var.computer_name_prefix}${module.suffix.result}" : null
